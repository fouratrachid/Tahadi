/**
 * Tahadi game store: a strict state machine.
 *
 ...
 * are no-ops. Timing is timestamp-based (turnEndsAt), so pause/resume and
 * backgrounding are exact and drift-free.
 */
...
interface GameState {
  ...
  // Round plan (one of these is set depending on the challenge)
  timedTurns: [Question[], Question[]] | null; // speed / reversed
  steps: Step[] | null; // whoAmI / ordering / bell

  // Runtime pointers
  turnPlayer: PlayerIndex; // active player during a timed turn
  qIndex: number; // index within the current timed turn OR into `steps`

  // Challenge-specific transient state
  revealedHints: number; // whoAmI: 1..4
  orderingRevealed: boolean; // ordering

  // Timing
  turnEndsAt: number | null; // absolute ms timestamp (null = untimed / paused)
  pausedRemainingMs: number | null;
  isPaused: boolean;

  // Question tracking
  usedIds: UsedIdsByPack; // persisted per pack across launches
  sessionUsed: Set<string>; // never repeat within a session
...
export const useGameStore = create<GameState>((set, get) => {
  /** Apply a transition and play the sound for entering a terminal phase. */
